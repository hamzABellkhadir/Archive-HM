...
public class JWTAuthorizationFiler extends OncePerRequestFilter{
	...
	response.addHeader("Access-Control-Allow-Methods","GET,POST,DELETE,PUT,PATCH");
		...
		else if(request.getRequestURI().equals("/logins")) {
		...
		DecodedJWT decodedJWT = verifier
				.verify(jwt.substring(SecurityParams.HEADER_PREFIX.length()));
		String pseudo = decodedJWT.getSubject();
		...
		UsernamePasswordAuthenticationToken user = new UsernamePasswordAuthenticationToken(pseudo, null,authorities);
